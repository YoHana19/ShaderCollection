...
)
{
    ...
    float ss_param = smoothstep(0, 1, param); // smoothstep(min, max, x) => x < min の場合は 0、x > max の場合は 1、x が [min, max] の範囲内であれば 0 と 1 の間の相対値を返します。

    uint seed = pid * 877;
    if (Random(seed) < 0.3) // ランダムに、2経路に分ける
    {
        // Triangle vertices at the relay point(中継地点)
