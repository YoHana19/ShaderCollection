...
Shader "Custom/DissolveEmissiveBorder" {
	Properties{
		_Color("Main Color", Color) = (.5,.5,.5,1)
		_MainTex("Base (RGB)", 2D) = "white" {}
		[HDR] _Emission1("Emission1", Color) = (.5,.5,.5,1)
		[HDR] _Emission2("Emission2", Color) = (.5,.5,.5,1)
		...
		SubShader{
			...
			Pass {
				...
				fixed4 frag(v2f i) : SV_Target
				{
					...
					fixed4 factor = fixed4(b, b, b, 1.0);
					return _Emission1 * factor;
				}
				ENDCG
			}
